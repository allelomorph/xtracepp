...
// common:
// BITGRAVITY
const std::vector<std::string_view> bitgravity {
     "Forget",     //  0
     "NorthWest",  //  1
     "North",      //  2
     "NorthEast",  //  3
...
// WINGRAVITY
const std::vector<std::string_view> wingravity {
     "Unmap",      //  0
     "NorthWest",  //  1
     "North",      //  2
     "NorthEast",  //  3
...
//BOOL
const std::vector<std::string_view> bool_ {
     "False",  // 0
     "True"    // 1
};
...
// SETofPOINTEREVENT
// SETofDEVICEEVENT
const std::vector<std::string_view> set_of_event {
     "KeyPress",              // 1 <<  0
     "KeyRelease",            // 1 <<  1
     "ButtonPress",           // 1 <<  2
     "ButtonRelease",         // 1 <<  3
...
// SETofKEYBUTMASK
const std::vector<std::string_view> set_of_keybutmask {
     "Shift",    // 1 <<  0
     "Lock",     // 1 <<  1
     "Control",  // 1 <<  2
     "Mod1",     // 1 <<  3
...
const std::vector<std::string_view> host_family {
    "Internet",           // 0
    "DECnet",             // 1
    "Chaos",              // 2
    "",
...
// image-byte-order 1B
const std::vector<std::string_view> image_byte_order {
    "LSBFirst",  // 0
    "MSBFirst"   // 1
};

// bitmap-format-bit-order 1B
const std::vector<std::string_view> bitmap_format_bit_order {
    "LeastSignificant",  // 0
    "MostSignificant"    // 1
};

// SCREEN.backing-stores 1B
const std::vector<std::string_view> screen_backing_stores {
    "Never",       // 0
    "WhenMapped",  // 1
    "Always"       // 2
};
...
// VISUALTYPE.class 1B
const std::vector<std::string_view> visualtype_class {
    "StaticGray",   // 0
    "GrayScale",    // 1
    "StaticColor",  // 2
    "PseudoColor",  // 3
...
const std::vector<std::string_view> window_class {
    "CopyFromParent",  // 0
    "InputOutput",     // 1
    "InputOnly"        // 2
};
...
const std::vector<std::string_view> zero_copy_from_parent {
    "CopyFromParent"  // 0
};

// Req 001 CreateWindow.value-mask 4B BITMASK
// Req 002 ChangeWindowAttributes.value-mask 4B BITMASK
const std::vector<std::string_view> window_attribute_value_mask {
...
const std::vector<std::string_view> window_attribute_background_pixmap {
    "None",           // 0
    "ParentRelative"  // 1
};
...
const std::vector<std::string_view> window_attribute_backing_store {
    "NotUseful",   // 0
    "WhenMapped",  // 1
    "Always"       // 2
};
...
const std::vector<std::string_view> zero_none {
    "None"  // 0
};

// Req 003 GetWindowAttributes - Reply.map-state 1B
const std::vector<std::string_view> window_attribute_map_state {
    "Unmapped",    // 0
    "Unviewable",  // 1
    "Viewable"     // 2
};
...
const std::vector<std::string_view> save_set_mode {
    "Insert",  // 0
    "Delete"   // 1
};
...
const std::vector<std::string_view> window_value_mask {
    "x",             // 1 << 0
    "y",             // 1 << 1
    "width",         // 1 << 2
    "height",        // 1 << 3
...
const std::vector<std::string_view> window_value_stack_mode {
    "Above",     // 0
    "Below",     // 1
    "TopIf",     // 2
    "BottomIf",  // 3
...
const std::vector<std::string_view> circulate_direction {
    "RaiseLowest",  // 0
    "LowerHighest"  // 1
};

// Req  018 ChangeProperty.mode 1B
const std::vector<std::string_view> change_property_mode {
    "Replace",  // 0
    "Prepend",  // 1
    "Append"    // 2
};
...
const std::vector<std::string_view> property_atom {
    "AnyPropertyType"  // 0
};

// Req 025 SendEvent.destination WINDOW
const std::vector<std::string_view> event_destination {
    "PointerWindow",  // 0
    "InputFocus"      // 1
};

// Req 028 GrabButton.button BUTTON
const std::vector<std::string_view> button {
    "AnyButton",  // 0
};

// Req 026 GrabPointer - Reply.status 1B
// Req 031 GrabKeyboard - Reply.status 1B
const std::vector<std::string_view> grab_status {
    "Success",         // 0
    "AlreadyGrabbed",  // 1
    "InvalidTime",     // 2
    "NotViewable",     // 3
...
const std::vector<std::string_view> input_mode {
    "Synchronous",  // 0
    "Asynchronous"  // 1
};
...
const std::vector<std::string_view> events_mode {
    "AsyncPointer",    // 0
    "SyncPointer",     // 1
    "ReplayPointer",   // 2
    "AsyncKeyboard",   // 3
...
const std::vector<std::string_view> time {
    "CurrentTime"  // 0
};
...
const std::vector<std::string_view> input_focus {
    "None",         // 0
    "PointerRoot",  // 1
    "Parent"        // 2
};
...
const std::vector<std::string_view> draw_direction {
    "LeftToRight",  // 0
    "RightToLeft"   // 1
};
...
const std::vector<std::string_view> gc_value_mask {
    "function",               // 1 <<  0
    "plane-mask",             // 1 <<  1
    "foreground",             // 1 <<  2
    "background",             // 1 <<  3
...
const std::vector<std::string_view> gc_value_function {
    "Clear",         //  0
    "And",           //  1
    "AndReverse",    //  2
    "Copy",          //  3
...
const std::vector<std::string_view> gc_value_line_style {
    "Solid",      // 0
    "OnOffDash",  // 1
    "DoubleDash"  // 2
};
...
const std::vector<std::string_view> gc_value_cap_style {
    "NotLast",    // 0
    "Butt",       // 1
    "Round",      // 2
    "Projecting"  // 3
...
const std::vector<std::string_view> gc_value_join_style {
    "Miter",  // 0
    "Round",  // 1
    "Bevel"   // 2
};
...
const std::vector<std::string_view> gc_value_fill_style {
    "Solid",          // 0
    "Tiled",          // 1
    "Stippled",       // 2
    "OpaqueStippled"  // 3
...
const std::vector<std::string_view> gc_value_fill_rule {
    "EvenOdd",  // 0
    "Winding"   // 1
};
...
const std::vector<std::string_view> gc_value_subwindow_mode {
    "ClipByChildren",   // 0
    "IncludeInferiors"  // 1
};
...
const std::vector<std::string_view> gc_value_arc_mode {
    "Chord",    // 0
    "PieSlice"  // 1
};

// Req 059 SetClipRectangles.ordering 1B
const std::vector<std::string_view> clip_rect_ordering {
    "UnSorted",  // 0
    "YSorted",   // 1
    "YXSorted",  // 2
    "YXBanded"   // 3
...
const std::vector<std::string_view> poly_shape {
    "Complex",    // 0
    "Nonconvex",  // 1
    "Convex"      // 2
};
...
const std::vector<std::string_view> poly_coordinate_mode {
    "Origin",   // 0
    "Previous"  // 1
};
...
const std::vector<std::string_view> image_format {
    "Bitmap",    // 0
    "XYPixmap",  // 1
    "ZPixmap"    // 2
};
...
const std::vector<std::string_view> colormap_alloc {
    "None",  // 0
    "All"    // 1
};
...
const std::vector<std::string_view> do_rgb_mask {
...
const std::vector<std::string_view> size_class {
    "Cursor",  // 0
    "Tile",    // 1
    "Stipple"  // 2
};
...
const std::vector<std::string_view> keyctl_value_mask {
...
const std::vector<std::string_view> off_on {
    "Off",  // 0
    "On"    // 1
};
...
const std::vector<std::string_view> screen_saver {
    "No",      // 0
    "Yes",     // 1
    "Default"  // 2
};
...
const std::vector<std::string_view> host_change_mode {
    "Insert",  // 0
    "Delete"   // 1
};

// Req 110 ListHosts - Reply.mode 1B
const std::vector<std::string_view> host_status_mode {
    "Disabled",  // 0
    "Enabled"    // 1
};

// Req 111 SetAccessControl.mode 1B
const std::vector<std::string_view> access_mode {
    "Disable",  // 0
    "Enable"    // 1
};

// Req 112 SetCloseDownMode.mode 1B
const std::vector<std::string_view> close_down_mode {
    "Destroy",          // 0
    "RetainPermanent",  // 1
    "RetainTemporary"   // 2
};
...
const std::vector<std::string_view> client_resource {
    "AllTemporary"  // 0
};

// Req 115 ForceScreenSaver.mode 1B
const std::vector<std::string_view> screen_saver_set_mode {
    "Reset",    // 0
    "Activate"  // 1
};
...
const std::vector<std::string_view> mapping_status {
    "Success",  // 0
    "Busy",     // 1
    "Failed"    // 2
};
...
const std::vector<std::string_view> motion_hint {
    "Normal",  // 0
    "Hint"     // 1
};
...
const std::vector<std::string_view> focus_mode {
    "Normal",       // 0
    "Grab",         // 1
    "Ungrab",       // 2
    "WhileGrabbed"  // 3
...
const std::vector<std::string_view> focus_same_screen_mask {
...
const std::vector<std::string_view> focus_detail {
    "Ancestor",          // 0
    "Virtual",           // 1
    "Inferior",          // 2
    "Nonlinear",         // 3
...
const std::vector<std::string_view> visibility_state {
    "Unobscured",         // 0
    "PartiallyObscured",  // 1
    "FullyObscured"       // 2
};
...
const std::vector<std::string_view> circulate_place {
    "Top",    // 0
    "Bottom"  // 1
};

// Evt  28 PropertyNotify.state 1B
const std::vector<std::string_view> property_state {
    "NewValue",  // 0
    "Deleted"    // 1
};

// Evt  32 ColormapNotify.state 1B
const std::vector<std::string_view> colormap_state {
    "Uninstalled",  // 0
    "Installed"     // 1
};

// Evt  34 MappingNotify.request 1B
const std::vector<std::string_view> mapping_state {
    "Modifier",  // 0
    "Keyboard",  // 1
    "Pointer"    // 2
};
