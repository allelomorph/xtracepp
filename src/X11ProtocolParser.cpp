...
size_t X11ProtocolParser::_logServerPacket(
    ...
    case Connection::AUTHENTICATION:
        ...
        break;
    case Connection::OPEN: {
        assert( sz >= protocol::errors::ENCODING_SZ &&
                sz >= protocol::requests::DEFAULT_REPLY_ENCODING_SZ &&
                sz >= protocol::events::ENCODING_SZ );
        // event codes with msb turned on are generated by request SendEvent
        const uint8_t prefix ( *data & _EVENT_CODE_MASK );
        switch ( prefix ) {
        case protocol::errors::PREFIX:
            bytes_parsed = _logError( conn, data, sz );
            assert( bytes_parsed == protocol::errors::ENCODING_SZ );
            break;
        case protocol::requests::REPLY_PREFIX:
            bytes_parsed = _logReply( conn, data, sz );
            assert( bytes_parsed >= protocol::requests::DEFAULT_REPLY_ENCODING_SZ );
            break;
        default:
            // TBD (2-35) will change with extensions
